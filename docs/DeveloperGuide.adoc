...
The User Interface is implemented using JavaFx. The Ui consists of a `MainWindow` which holds `StackWindow` fields.
Each of the `StackWindow` is to be filled with the Ui components as listed below.

*Ui components:* +

* `CommandBox` - Input field
* `PersonListPanel` - Display list of Person
* `ResultDisplay` - Display response of command
* `PersonDetailsCard` - Display Details of Person
* `AddressPanel` - Display Address and shows map of address

The Ui also makes use of `DarkTheme,css` and `LightTheme.css` file to set the color scheme.
...
=== Recycle bin [yellow]#*Coming in v2.0#
The recycle bin is implemented by having address book store the list of person deleted using a separate list from the main address book list. +
In this list, the person details is stored along with a date of deletion of the person. +
Upon every startup of the program, the system time is checked against the date of deletion, and entries that are more than 7 days old will be removed from the list. +

==== Design considerations

**Aspect:** Implementation of `RecycleBin` +
**Alternative 1 (current choice):** Add a new type of list `RecycleBinList` extending from `UniquePersonsList` +
**Pros:** We will be able to store additional information in the list such as deletion date. Existing methods for `UniquePersonsList` can still be used. +
**Cons:** Increases workload as a new class is added. Testing of newly introduced class is also required. +
**Alternative 2:** Just reuse `UniquePersonsList` to store a separate list for `recycleBin` +
**Pros:** Does not require alot of code changes. +
**Cons:** Unable to add additional fields to `recycleBin` entry. Additional features such as automatic removal of entry from list cannot be implemented without changing existing code in `UniquePersonsList`. +
...
**Cons:** A new parser will have to added that determines the command is for `RecycleBin` before command is to be parsed again to determine usage. +
**Alternative 2:** Add a new command for each feature of `RecycleBin` usage. +
**Pros:** Additional parsing of command is unnecessary. +
**Cons:** Commands will be easily mixed up. Purpose of command will be unintuitive, `clear` may mean to clear recycle or to clear address book. A separate command name for clearing recycle bin need to be thought of (clear can no longer be used). +
...
* `name` +
* `address` +
* `email` (each keyword must contain a match) +
* `phone number` +
...
The fields that are not compulsory are `phone`, `birthday`, `email`, `address`, and `tags`.
