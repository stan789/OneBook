...
**Alternative 1 (current choice):** Add a new type of list `RecycleBinList` extending from `AddressBook` +
**Pros:** We will be able to store additional information in the list such as deletion date. Existing methods for `AddressBook` can still be used. +
...
**Cons:** Unable to add additional fields to `recycleBin` entry. Additional features such as automatic removal of entry from list cannot be implemented without changing existing code in `AddressBook`. +
